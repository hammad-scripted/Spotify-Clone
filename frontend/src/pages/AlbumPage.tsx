...
export const AlbumPage = () => {
  ...
  return <div className="min-h-screen bg-[#07070a] pb-28 text-white selection:bg-lime-300 selection:text-black">
...
function AlbumState({ icon: Icon, title, copy, spin }: { icon: typeof Disc3; title: string; copy: string; spin?: boolean }) { return <div className="grid min-h-screen place-content-center justify-items-center bg-[#07070a] px-6 text-center text-white"><span className="mb-5 grid size-14 place-items-center rounded-2xl bg-white/[.04] text-violet-300 ring-1 ring-white/10"><Icon className={cn('size-6', spin && 'animate-spin')} /></span><h1 className="text-lg font-semibold">{title}</h1><p className="mt-2 text-xs text-zinc-600">{copy}</p><Link to="/" className="mt-6 text-xs text-zinc-500 hover:text-white">Return home</Link></div>; }
