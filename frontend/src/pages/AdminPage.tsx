...
function AdminState({ icon: Icon, title, copy, action, spin }: { icon: typeof ShieldCheck; title: string; copy: string; action?: React.ReactNode; spin?: boolean }) { return <div className="app-shell motion-page grid min-h-screen place-content-center justify-items-center bg-[#07070a] px-6 text-center text-white"><ThemeToggle className="fixed right-5 top-5" /><span className="mb-5 grid size-14 place-items-center rounded-2xl border border-white/10 bg-white/[.04] text-violet-300"><Icon className={cn('size-6', spin && 'animate-spin')} /></span><h1 className="text-xl font-semibold tracking-tight">{title}</h1><p className="mb-6 mt-2 max-w-sm text-xs leading-5 text-zinc-600">{copy}</p>{action}<Link to="/" className="mt-5 text-[10px] text-zinc-600 hover:text-white">Return to Soundwave</Link></div>; }
